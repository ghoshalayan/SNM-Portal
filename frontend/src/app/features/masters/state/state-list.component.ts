      ...
      <mat-card-header>
        ...
        <div class="header-actions">
          <mat-form-field appearance="outline" class="filter-field">
            ...
            <mat-select [(value)]="selectedCountry" (selectionChange)="load()">
          ...
          <mat-form-field appearance="outline" class="search-field">
            ...
            <input matInput (keyup)="applyFilter($event)" placeholder="Search states..." />
      ...
      <mat-card-content>
        ...
        <table mat-table [dataSource]="dataSource" matSort class="full-width" [hidden]="loading">
          <ng-container matColumnDef="stateid">
            ...
            <td mat-cell *matCellDef="let row">{{ row.stateid }}</td>
          </ng-container>
...
export class StateListComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns = ['stateid', 'StateName', 'Country', 'actions'];
  loading = true;
  countries: any[] = [];
  selectedCountry = '';
  ...
  load() {
    ...
    this.api.get('/masters/states', params).subscribe({
      ...
      error: () => {
        const remaining = Math.max(0, 500 - (Date.now() - start));
        setTimeout(() => { this.notify.error('Failed to load states'); this.loading = false; }, remaining);
      },
    });
  }
  ...
  openDialog(row?: any) {
    const ref = this.dialog.open(StateDialogComponent, { width: '480px', data: { row: row || null, countries: this.countries } });
    ref.afterClosed().subscribe((saved) => { if (saved) this.load(); });
  }

  deleteItem(row: any) {
    const ref = this.dialog.open(ConfirmDialogComponent, { data: { message: `Delete state "${row.StateName}"?` } });
    ref.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api.delete(`/masters/states/${row.stateid}`).subscribe({
