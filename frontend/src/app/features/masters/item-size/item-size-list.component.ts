...
import { ItemSizeDialogComponent } from './item-size-dialog.component';
...
@Component({
  ...
  template: `
    <mat-card>
      <mat-card-header>
        <mat-card-title>Item Sizes</mat-card-title>
        <div class="header-actions">
          <mat-form-field appearance="outline" class="search-field">
            <mat-label>Search</mat-label>
            <input matInput (keyup)="applyFilter($event)" placeholder="Search item sizes..." />
            <mat-icon matSuffix>search</mat-icon>
          </mat-form-field>
          <button mat-raised-button color="primary" (click)="openDialog()">
            <mat-icon>add</mat-icon> Add Item Size
          </button>
        </div>
      </mat-card-header>
      <mat-card-content>
        ...
        <table mat-table [dataSource]="dataSource" matSort class="full-width" [hidden]="loading">
          <ng-container matColumnDef="itemSizeId">
            <th mat-header-cell *matHeaderCellDef mat-sort-header>#</th>
            <td mat-cell *matCellDef="let row">{{ row.itemSizeId }}</td>
          </ng-container>
          ...
          <ng-container matColumnDef="itemSize">
            <th mat-header-cell *matHeaderCellDef mat-sort-header>Item Size</th>
            <td mat-cell *matCellDef="let row">{{ row.itemSize }}</td>
          </ng-container>
...
export class ItemSizeListComponent implements OnInit, AfterViewInit {
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns = ['itemSizeId', 'itemName', 'itemSize', 'actions'];
  loading = true;
  ...
  load() {
    ...
    this.api.get('/masters/item-sizes').subscribe({
      next: (res: any) => {
      ...
      error: () => {
        const remaining = Math.max(0, 500 - (Date.now() - start));
        setTimeout(() => { this.notify.error('Failed to load item sizes'); this.loading = false; }, remaining);
      },
    });
  }
  ...
  openDialog(row?: any) {
    const ref = this.dialog.open(ItemSizeDialogComponent, {
      width: '480px',
      data: row || null,
    });
  ...
  deleteItem(row: any) {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: { message: `Delete item size "${row.itemSize}"?` },
    });
    ref.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api.delete(`/masters/item-sizes/${row.itemSizeId}`).subscribe({
