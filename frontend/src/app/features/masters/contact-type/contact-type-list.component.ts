...
import { ContactTypeDialogComponent } from './contact-type-dialog.component';
...
@Component({
  ...
  template: `
    <mat-card>
      <mat-card-header>
        <mat-card-title>Contact Types</mat-card-title>
        <div class="header-actions">
          <mat-form-field appearance="outline" class="search-field">
            <mat-label>Search</mat-label>
            <input matInput (keyup)="applyFilter($event)" placeholder="Search contact types..." />
            <mat-icon matSuffix>search</mat-icon>
          </mat-form-field>
          <button mat-raised-button color="primary" (click)="openDialog()">
            <mat-icon>add</mat-icon> Add Contact Type
          </button>
        </div>
      </mat-card-header>
      <mat-card-content>
...
export class ContactTypeListComponent implements OnInit, AfterViewInit {
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns = ['contactTypeId', 'contactType', 'actions'];
  loading = true;
  ...
  load() {
    ...
    this.api.get('/masters/contact-types').subscribe({
      next: (res: any) => {
      ...
      error: () => {
        const remaining = Math.max(0, 500 - (Date.now() - start));
        setTimeout(() => { this.notify.error('Failed to load contact types'); this.loading = false; }, remaining);
      },
    });
  }
  ...
  openDialog(row?: any) {
    const ref = this.dialog.open(ContactTypeDialogComponent, {
      width: '480px',
      data: row || null,
    });
  ...
  deleteItem(row: any) {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: { message: `Delete contact type "${row.contactType}"?` },
    });
    ref.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api.delete(`/masters/contact-types/${row.contactTypeId}`).subscribe({
