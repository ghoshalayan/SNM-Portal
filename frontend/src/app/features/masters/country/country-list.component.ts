      ...
      <mat-card-header>
        ...
        <div class="header-actions">
          <mat-form-field appearance="outline" class="search-field">
            ...
            <input matInput (keyup)="applyFilter($event)" placeholder="Search countries..." />
...
export class CountryListComponent implements OnInit, AfterViewInit {
  ...
  displayedColumns = ['countryid', 'countryname', 'actions'];
  loading = true;
  ...
  load() {
    ...
    this.api.get('/masters/countries').subscribe({
      ...
      error: () => {
        const remaining = Math.max(0, 500 - (Date.now() - start));
        setTimeout(() => { this.notify.error('Failed to load countries'); this.loading = false; }, remaining);
      },
    });
  }
  ...
  openDialog(row?: any) {
    const ref = this.dialog.open(CountryDialogComponent, { width: '480px', data: row || null });
    ref.afterClosed().subscribe((saved) => { if (saved) this.load(); });
  }

  deleteItem(row: any) {
    const ref = this.dialog.open(ConfirmDialogComponent, { data: { message: `Delete country "${row.countryname}"?` } });
    ref.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api.delete(`/masters/countries/${row.countryid}`).subscribe({
