...
import { ItemNameDialogComponent } from './item-name-dialog.component';
import { SkeletonLoaderComponent } from '../../../shared/components/skeleton-loader/skeleton-loader.component';

@Component({
  selector: 'app-item-name-list',
  standalone: true,
  imports: [
    CommonModule,
    MatTableModule,
  ...
  template: `
    <mat-card>
      <mat-card-header>
        <mat-card-title>Item Names</mat-card-title>
        <div class="header-actions">
          <mat-form-field appearance="outline" class="search-field">
            <mat-label>Search</mat-label>
            <input matInput (keyup)="applyFilter($event)" placeholder="Search item names..." />
            <mat-icon matSuffix>search</mat-icon>
          </mat-form-field>
          <button mat-raised-button color="primary" (click)="openDialog()">
            <mat-icon>add</mat-icon> Add Item Name
          </button>
        </div>
      </mat-card-header>
      <mat-card-content>
        @if (loading) { <app-skeleton-loader type="table" [rows]="5" [columns]="8"></app-skeleton-loader> }
        <table mat-table [dataSource]="dataSource" matSort class="full-width" [hidden]="loading">
          ...
          <ng-container matColumnDef="erpItemCode">
            <th mat-header-cell *matHeaderCellDef mat-sort-header>ERP Code</th>
            <td mat-cell *matCellDef="let row">{{ row.erpItemCode }}</td>
          </ng-container>
          <ng-container matColumnDef="erpName">
            <th mat-header-cell *matHeaderCellDef mat-sort-header>ERP Name</th>
            <td mat-cell *matCellDef="let row">{{ row.erpName }}</td>
          </ng-container>
...
export class ItemNameListComponent implements OnInit, AfterViewInit {
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns = ['itemId', 'itemGradeName', 'itemName', 'itemDia', 'itemLength', 'erpItemCode', 'erpName', 'actions'];
  loading = true;
  ...
  load() {
    ...
    this.api.get('/masters/item-names').subscribe({
      ...
      error: () => {
        const remaining = Math.max(0, 500 - (Date.now() - start));
        setTimeout(() => { this.notify.error('Failed to load item names'); this.loading = false; }, remaining);
      },
    });
  }
  ...
  openDialog(row?: any) {
    const ref = this.dialog.open(ItemNameDialogComponent, {
      width: '560px',
      data: row || null,
    });
  ...
  deleteItem(row: any) {
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: { message: `Delete item name "${row.itemName}"?` },
    });
    ref.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api.delete(`/masters/item-names/${row.itemId}`).subscribe({
