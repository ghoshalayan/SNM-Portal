...
export interface EnqFollowUp {
  engfollowupid?: number;
  enqid?: number;
...
@Component({
  ...
  template: `
    <h2 mat-dialog-title>{{ data.followup?.engfollowupid ? 'Edit Follow-Up' : 'Add Follow-Up' }}</h2>
    <mat-dialog-content>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-raised-button color="primary" (click)="save()" [disabled]="form.invalid">
        {{ data.followup?.engfollowupid ? 'Update' : 'Add' }}
      </button>
    </mat-dialog-actions>
  `,
  styles: [`
...
export class FollowUpDialogComponent {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { followup: EnqFollowUp | null }
  ) {
  ...
  template: `
    <div class="followup-container">
      <div class="followup-toolbar">
        ...
        <button mat-raised-button color="primary" (click)="openDialog(null)" [disabled]="!enqId">
...
export class EnquiryFollowUpComponent implements OnInit, OnChanges {
  @Input() enqId!: number | null;

  displayedColumns = ['followupdate', 'followupmode', 'followupremarks', 'nextfollowupdate', 'createdon', 'actions'];
  dataSource = new MatTableDataSource<EnqFollowUp>([]);
  isLoading = false;
  ...
  ngOnInit(): void {
    if (this.enqId) this.loadFollowUps();
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['enqId'] && this.enqId) this.loadFollowUps();
  }
  ...
  loadFollowUps(): void {
    ...
    this.isLoading = true;
    this.api.get<EnqFollowUp[]>(`/enquiries/${this.enqId}/followups`).subscribe({
      next: (data) => {
  ...
  openDialog(followup: EnqFollowUp | null): void {
    ...
    dialogRef.afterClosed().subscribe((result: Partial<EnqFollowUp> | null) => {
      if (!result) return;

      if (followup?.engfollowupid) {
        this.api
          .put(`/enquiries/${this.enqId}/followups/${followup.engfollowupid}`, result)
          .subscribe({
            next: () => {
        ...
        this.api
          .post(`/enquiries/${this.enqId}/followups`, result)
          .subscribe({
            next: () => {
  ...
  deleteFollowUp(followup: EnqFollowUp): void {
    ...
    dialogRef.afterClosed().subscribe((confirmed) => {
      if (confirmed) {
        this.api
          .delete(`/enquiries/${this.enqId}/followups/${followup.engfollowupid}`)
          .subscribe({
            next: () => {
